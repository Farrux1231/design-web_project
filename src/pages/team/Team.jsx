...
const Team = () => {
  return (
    <section>
      <div className="h-[38vh] pl-[13vw] ">
        <div className="max-w-[510px] ml-[8%] mt-[7%]">
          <h2 className="text-[40px] font-serif text-gray-900 mb-3 relative">TEAM</h2>
          <p className="text-gray-400 text-[15px] leading-relaxed font-medium w-[450px]">
...
<div className='bg-[#f8f6f6]'>
  <div className="pl-[200px] pr-10 space-y-12 mt-10 w-[93%] pt-4 pb-7 ">
    {sections.map((section, sIndex) => (
      <div key={sIndex}>
        <h3 className="text-4xl font text-gray-800 font-serif mb-4 ">{section.title}</h3>
        
        <div className="flex gap-[20px] flex-wrap ">
          {section.members.map((member, index) => (
            <div
              key={index}
              className="relative group w-[260px] h-[400px] overflow-hidden"
              style={{
                boxShadow: 'inset 0px -40px 40px rgba(0, 0, 0, 10)',
              }}
            >
              <img
                src={member.image}
              ...
              <div className="absolute bottom-[-18px] left-0 w-full px-4 text-center z-20 ">
                <div className="transition-all duration-500 transform group-hover:-translate-y-[130px]">
                  <h3 className="text-[24px] font-bold text-white">{member.name}</h3>
                  <p className="text-sm text-[20px] text-white">{member.position}</p>
